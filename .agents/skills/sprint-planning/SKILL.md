...
Before committing an item to the sprint, check it against the **`definition-of-ready`**
skill. Only commit 🟢 **Ready to start** items; pull in a 🟡 **Almost ready** item
only if its single named blocker is resolved during planning; **never commit**
🔴 **Not ready** items (missing AC/design, incomplete scoring, unscoped épics,
unrealistic estimates) — send them back to refinement instead. If the backlog was
audited (see `jira-ticket-audit`), each ticket already carries its DoR verdict.
