---
name: ticket-triage
description: Triage and prioritize an inbound SUPPORT ticket or customer issue (not a product/backlog item). Use when a new support ticket comes in and needs categorization, assigning P1-P4 priority, deciding which team should handle it, or checking whether it's a duplicate or known issue before routing. For auditing or refining product backlog tickets (readiness, scoring, sprint prep) use jira-ticket-audit instead.
