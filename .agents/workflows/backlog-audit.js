...
5. Extract EVERY external URL (figma.com, notion.so, slack.com, github.com) from each ticket and its linked tickets, and fetch each verbatim: notion.so → notion-fetch (mcp__claude_ai_Notion__ or mcp__notion__ prefix, whichever is on your tool list; quote requirements/AC, decisions, open questions); slack.com → slack_read_thread (mcp__claude_ai_Slack__ or mcp__slack__ prefix; quote relevant messages); github.com → \`gh pr/issue view\` via Bash (quote title/state/body); figma.com → get_metadata (mcp__claude_ai_Figma__ or mcp__figma__ prefix). Mark anything unreachable as "unreadable".
